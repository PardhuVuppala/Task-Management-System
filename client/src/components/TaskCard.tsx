...
function TaskCard({task, deleteTask,updateTask} : Props) {
    ...
    if(isDragging)
    {
        return <div ref={setNodeRef} style={style}  className='bg-mainBackgroundColor
        p-2.5 opacity-50
        h-[100px] items-center flex text-left rounded-xl border-2 border-rose-500  cursor-grabs relative'>
        </div>
    }
    if(editMode){
        return  ( 
        ...
        p-2.5
        h-[100px] items-center flex text-left rounded-xl hover:ring-2 hover:ring-inset 
        hover:ring-rose-500 cursor-grabs relative task' >
  ...
  return  (
    ...
    p-2.5
    h-[100px] items-center flex text-left rounded-xl hover:ring-2 hover:ring-inset 
    hover:ring-rose-500 cursor-grabs relative' onMouseEnter={()=>setMouseIsOver(true)} onMouseLeave={()=>setMouseIsOver(false)}>
   ...
   className='stroke-white absolute right-4 top-1/2
    -translate-y-1/2 bg-columnBackgroundColor p-2 rounded opacity-60 hover:opacity-100'><TrashIcon/>
    </button>)}</div>
  )
}
